...
// Добавим начальное значение stateв конструктор:
  constructor(props) {
    ...
    this.state = {
      ...
      citi: "",
      index: "",
      formErrors: { index: "", citi: "", address: "", nams: "" },
      formValid: false,
      namsValid: false,
      addressValid: false,
      citiValid: false,
...
//   валидатор
  validateField(fieldName, value) {
    ...
    let citiValid = this.state.citiValid;
    let indexValid = this.state.indexValid;

    switch (fieldName) {
      case "index":
        indexValid = value.length >= 5;
        fieldValidationErrors.index = indexValid ? "" : " is invalid";
        break;

      case "citi":
        citiValid = value.length >= 5;
        fieldValidationErrors.citi = citiValid ? "" : " is invalid";
        break;
          ...
          <Col  lg={4}>
            <Container>
              <Row className="form-margin-main">
                <p className="form-text-title">Доставка</p>
                <img src={Arrow} alt="Arrow" />
                <p className="form-text-title-next">Оплата</p>
                <h2>Информация для доставки </h2>
              </Row>

              <form className="form-order" action="/step-two">
                ...
                >
                  <label htmlFor="nams">Получатель</label>
                  <input
                    required
                ...
                <div
                  className={`mt-4 pt-2 form-group form-margin-main ${this.errorClass(
                    this.state.formErrors.citi
                  )}`}
                >
                  <label htmlFor="citi">Адрес</label>
                  <input
                    required
                    className="form-control "
                    name="citi"
                    placeholder="Город"
                    value={this.state.citi}
                    onChange={this.handleUserInput}
                  />
                </div>
                ...
                <select name="country" required="" className="form-control">
                   <optgroup>
                  		<option data-countryCode="00" value="+000">Страна</option>
                ...
                >
                  ...
                  <input
                    required
                    className="form-control form-index-media "
                    name="index"
                    placeholder="Индекс"
